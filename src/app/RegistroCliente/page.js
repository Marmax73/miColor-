...
export default function RegistroCliente() {
  const [form, setForm] = useState({ nombre: '', apellido: '', localidad: '', numeroDeMobil: '' });
  ...
  const handleSubmit = async e => {
    ...
    const res = await fetch('/api/userCliente', {
      method: 'POST',
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto p-4 space-y-4">
      ...
      <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded">Registrar Cliente</button>
      <hr />
      <TextoIndicativo />
    </form>
  );
